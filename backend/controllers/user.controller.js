...
export async function sendFriendRequest(req, res) {
    try {
        ...
        // we have built the logic to not send request to urself but just in case
        if(myId === recipientId) {
        ...
        if(existingRequest) {
            return res.status(400).json({message: "A friends request already exists between you and the user"});
        }
...
export async function acceptFriendRequest(req, res) {
    try {
        ...
        // add each users id to the other friends array
